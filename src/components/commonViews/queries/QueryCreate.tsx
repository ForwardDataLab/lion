...
import fakeSchema from "../../../data/fakeSchema.json";
...
import {queryCreateStyle} from "../../../styles/queryStyle";
...
export function QueryCreate(props: QueryCreateProps) {
    ...
    const [schema, setSchema] = useState({} as JSONObject);
                ...
                <Grid container spacing={4}>
                    ...
                    <Grid item xs={3}>
                        <FormControl color={'secondary'}>
                            <InputLabel id="select-source-label">Age</InputLabel>
                            <Select
                                labelId="select-source-label"
                                value={allSources[0]}
                                onChange={onUpdateSource}>
                                {
                                    allSources.map(source => (<MenuItem value={source}>{source}</MenuItem>))
                                }
                            </Select>
                        </FormControl>
                    </Grid>
                    <Grid item xs={3}>
                        <FormControl color={'secondary'}>
                            <InputLabel id="select-schedule-label">Age</InputLabel>
                            <Select
                                labelId="select-schedule-label"
                                value={allSchedules[0]}
                                onChange={onUpdateSchedule}>
                                {
                                    allSchedules.map(schedule => (<MenuItem value={schedule}>{schedule}</MenuItem>))
                                }
                            </Select>
                        </FormControl>
                    </Grid>
